...
//监听路由实现视图切换
//这样写是为了方便扩展其他路由
        //类似 view.hone 的路由会触发 onRouteChange 方法
...
//获取目标视图名称，如果找不到则返回 404 页面
            view = (node && node.get('viewType')) || 'page404',
//获取上一视图
            lastView = me.lastView,
            //检查目标视图是否已经存在
            existingItem = mainCard.child('component[routeId=' + hashTag + ']'), newView;
        //如果上一个视图存在则触发这个视图的自定义事件 viewHide
        //扩展监听，有些时候可能会用到
        if (lastView) {
            lastView.fireEvent('viewHide', lastView);
        }
        //如果上个视图存在，并且是 Window 视图
        ...
        if (!existingItem) {
            newView = Ext.create({
                xtype: view,
                // 用户视图切换时搜索查找
                routeId: hashTag, hideMode: 'offsets'
            });
        }
        //如果新视图不存在，或者新视图不是 Window 视图
...
//容器中加入视图
        ...
        //获取焦点 //if (newView.isFocusable(true)) { // newView.focus(); //} //将当前视图保存到 lastView
        me.lastView = newView;
    },
    //折叠或展开导航树
...
//导航菜单是否折叠
            collapsing = !navigationList.getMicro(), new_width = collapsing ? 64 : 250;
...
//设置菜单栏折叠状态
            ...
            //恢复布局
            Ext.resumeLayouts();
            // do not flush the layout here... // 低版本浏览无动画效果
