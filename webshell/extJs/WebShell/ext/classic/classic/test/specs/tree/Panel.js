...
topSuite("Ext.tree.Panel", [
    ...
    describe("glyphs for icons", function() {
        it("should render glyphs using the specified font", function() {
            ...
            expect(Ext.fly(view.getCellByPosition({
                row: 0,
                column: 0
            }, true).querySelector('.x-tree-icon')).getStyle('font-family')).toBe('FontAwesome');

            // Check that the glyph is the first character of the text.
            expect(view.getCellByPosition({
                row: 0,
                column: 0
            }, true).querySelector('.x-tree-icon').innerHTML).toBe(store.getAt(0).get('text').substr(0, 1));
        });
    });
